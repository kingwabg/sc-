...
/**
 * ApprovalLinePanel — 결재선 시각화 (대기/승인/반려 색상)
 *
 * P11-1의 app/approval/doc/[id]/_components/ApprovalLinePanel.tsx와 별도 namespace
 */
import { cn } from "@/lib/utils";
import { CheckCircle2, XCircle, Clock } from "lucide-react";
import type { ApprovalStep } from "@/lib/features/approval/types";
...
const STEP_LABEL: Record<ApprovalStep["status"], string> = {
  pending:  "대기",
  current:  "결재중",
  approved: "승인",
  rejected: "반려",
...
interface Props {
  req: import("@/lib/features/approval/types").ApprovalRequest;
}

export function ApprovalLinePanel({ req }: Props) {
  return (
          ...
          return (
            <li key={step.step} className="flex gap-3">
              ...
              <div className="flex flex-col items-center">
                <div className={cn("w-9 h-9 rounded-full grid place-items-center ring-2", tone.bg, tone.ring)}>
              ...
              <div className="flex-1 pt-1.5 pb-4">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={cn("inline-block text-[10px] px-1.5 py-0.5 rounded font-semibold", tone.bg, tone.text)}>
                    {STEP_LABEL[step.status]}
                  </span>
                  <span className="text-[10px] text-slate-400 ml-auto">{step.step}단계</span>
                </div>
                {step.comment && (
                ...
                {step.actedAt && (
                  <div className="text-[10px] text-slate-400 mt-1">{formatDate(step.actedAt)}</div>
                )}
              </div>
            </li>
          );
